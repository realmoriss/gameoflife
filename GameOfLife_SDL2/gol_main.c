 ...
 * Filename:        gol_main.c
 * Description:     A jatek fobb fuggvenyei, az inicializalas es az allapotgep
 *
...
static const char FILENAME_SETTINGS[] = "save/settings.bin";
...
GameState Game_StateMachine(GameVars *game_vars) {
    ...
    case STATE_SIM_SAVE_CONF:
        ...
        return events_menu_save_conf(game_vars);
...
int Game_Init_All(GameVars *game_vars, const char *title) {
    ...
    game_vars->game_font = Game_LoadFont(game_vars->renderer, GAME_FONT_PATH, "FONT LOADED", (SDL_Color) {
        0xFF, 0xFF, 0xFF, 0xFF
    }, GAME_FONT_SIZE);
